...
class SimplePingAdapter: NSObject, SimplePingDelegate {
    ...
    func simplePing(_ pinger: SimplePing, didReceiveUnexpectedPacket packet: Data) {
        ...
        if let string = String(data: packet, encoding: .utf8) {
            print("UnexpectedPacket: ", string)
        } else {
            print("UnexpectedPacket: ", packet as NSData)
        }
        delegate?.didFailPingWithError(NSError(domain: "ping", code: -1, userInfo: [NSLocalizedDescriptionKey : "unexpected packet", "packet" : packet]))
        stopPinging()
    }
